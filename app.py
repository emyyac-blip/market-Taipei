...
# 資料讀取 (內建自動補欄位功能)
def load_data(file, file_type="sales"):
    if os.path.exists(file):
        try:
            df = pd.read_csv(file)
            if file_type == "sales" and not df.empty:
                if '獲得點數' not in df.columns: df['獲得點數'] = 0
                if '小時' not in df.columns: df['小時'] = datetime.now().hour
            return df
...
history_df = load_data(SALES_FILE, "sales")
stock_df = load_data(STOCK_FILE, "stock")
...
# 庫存計算邏輯
def get_rem_stock(s_df, h_df):
    s_map = s_df.set_index("品項")[["初始庫存", "補貨量"]].sum(axis=1).to_dict()
    if not h_df.empty:
        for _, row in h_df.iterrows():
            try:
                for itm in eval(row['明細']):
                    name = itm.split('] ')[1].split('x')[0]
                    if name in s_map: s_map[name] -= int(itm.split('x')[1])
            except: pass
    return s_map

# 分頁介面
t1, t2, t3 = st.tabs(["💰 結帳櫃檯", "📦 庫存管理", "📊 業績分析"])
...
with t1:
    ...
    c1, c2 = st.columns(2)
    with c1: c_name = st.text_input("👤 客戶名稱", value=f"現場客 {n_num}")
    with c2: dsc = st.number_input("💸 折扣", min_value=0, value=0, step=5)
    
    st.write("---")
    curr_i = get_rem_stock(stock_df, history_df)
    ca, cb, cc = st.columns([1, 2, 1])
    with ca: cat = st.selectbox("📂 分類", list(product_catalog.keys()))
    with cb:
        i_list = list(product_catalog[cat].keys())
        d_list = [f"{i} (餘 {curr_i.get(i, 0)})" for i in i_list]
        s_item = st.selectbox("🍎 品項", d_list).split(" (餘")[0]
        rem = curr_i.get(s_item, 0)
    with cc:
    ...
    if st.button("✅ 結帳完成", type="primary", use_container_width=True):
        if st.session_state.cart:
            new_r = {"時間": datetime.now().strftime("%H:%M:%S"), "客戶": c_name, "明細": str([f"{i['品項']}x{i['數量']}" for i in st.session_
