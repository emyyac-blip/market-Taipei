...
st.set_page_config(page_title="花田喜彘 POS 最終版", layout="wide")
...
c1, c2 = st.columns(2)
with c1: customer_name = st.text_input("👤 客人名稱", value=default_name)
...
if st.button("✅ 結帳完成 / 下一單", type="primary", use_container_width=True):
    if st.session_state.cart:
        ...
        order_info = {"單號": next_num, "時間": datetime.now().strftime("%H:%M:%S"), "客戶": customer_name, "明細": str(items_msg), "實收": final_total, "獲得點數": int(earned_points)}
...
if st.session_state.history:
    st.subheader(f"📊 今日結帳：{len(st.session_state.history)} 筆")
    df_h = pd.DataFrame(st.session_state.history)
    st.table(df_h[['單號', '時間', '客戶', '明細', '實收', '獲得點數']])
